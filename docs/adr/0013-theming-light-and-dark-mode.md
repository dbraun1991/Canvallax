...
- Status: Accepted — first-visit default superseded by [ADR-0020](0020-default-theme-is-light.md): light regardless of OS preference, not `prefers-color-scheme`-based as described below. Interaction Canvas's theming mechanism superseded by [ADR-0021](0021-interaction-canvas-excalidraw.md): Excalidraw's own `theme` prop, not draw.io's `dark: true` postMessage payload. System/Integration's `dark: theme === 'dark'` mechanism superseded by [ADR-0024](0024-system-canvas-drawing-surface-stays-light.md): its drawing surface now stays permanently light, like Process. Everything else in this ADR is unchanged.
- Date: 2026-09-03

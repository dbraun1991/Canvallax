...
| visible   |    Presenting mode: click enlarges the tile in a zoom       |
| once an   |    lightbox instead — no live editor mounts, no tabs shown. |
| Issue is  |                                                              |
| active,   |                                                              |
| resizable,|                                                              |
| footer:   |                                                              |
...
| `docs/adr/0001-*.md` – `0018-*.md` | Individual decisions — see the index for titles |
...
| [0018](docs/adr/0018-presenting-and-editing-canvas-modes.md) | Presenting mode: click a canvas tile to enlarge it. Editing mode: click enters it, tab-styled switcher |
...
| Path | Role | ADR |
|------|------|-----|
| `src/shell/shell-state.js` | Alpine data factory: Issue selection/picker, view switching, canvas mode, Backlog panel state, resize, theme, history, copy | 0002, 0017, 0018 |
...
| `index.html` | Markup + Alpine directives for the whole shell | 0002, 0017, 0018 |
...
- **File-manager-style Issue picker.** The Issue-picker overlay's list (ADR-0017) is still a flat, unfiltered list beyond its search field — worth revisiting (tabs, folder tree, per `bpmn-process-creator`'s own sidebar) once there are enough Issues that a flat list stops scaling.
...
- **Zoom lightbox is a static enlarge, not a live viewer.** Presenting mode's zoomed tile (ADR-0018) reuses the same rendered thumbnail, not a pannable/zoomable live render — fine for "look closer," a real diagram viewer is a bigger feature if that turns out to matter.
